...
void malloc_managed_monolithic(std::string input_file, vertex num_nodes,
  ...
  mmapPopulateEdgeArray(input_file, d_edges, num_edges, num_nodes, generate_random_weights);
...
void new_boruvka(std::string filename, std::string result_filename, 
    ...
    if(debug){
      ...
      if(use_malloc_managed){
        malloc_managed_monolithic(filename, num_nodes, num_edges, 
